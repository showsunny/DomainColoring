    ...
    if continual_gradient:
        ...
        if phase_contour_increase==True:
            if modulus_contour_increase == True:
                Brightness = np.clip(0.6 + 0.5*Brightness_modulus_contour+0.5*Brightness_phase_contour, 0, 1)
            else:
                if modulus_contour_decrease == True:
                    Brightness = np.clip(0.6 + 0.5*(1-Brightness_modulus_contour)+0.5*Brightness_phase_contour, 0, 1) 
                else:
                    Brightness = np.clip(0.6+Brightness_phase_contour, 0, 1)
        else:
            if phase_contour_decrease==True:
                if modulus_contour_increase == True:
                    Brightness = np.clip(0.6 + 0.5*Brightness_modulus_contour+0.5*(1-Brightness_phase_contour), 0, 1) 
                else:
                    if modulus_contour_decrease == True:
                        np.clip(0.6 + 0.5*(1-Brightness_modulus_contour)+0.5*(1-Brightness_phase_contour), 0, 1)
                    else:
